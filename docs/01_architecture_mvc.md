...
## 5b. 画面構成マップ（実アプリ準拠 / Web・PWA）

ボトムナビ5タブ構成（pato 実アプリに合わせる）。MVPは太字を優先実装。

```
[探す]      キャスト一覧・絞り込み検索（エリア/年齢/身長/タグ/クラス）、在席/合流中表示、
            つぶやきフィード
[メッセージ] スレッド一覧（すべて/お気に入り/非表示・ニックネーム検索・キャンペーンバナー）
            → 呼び出し単位のグループチャット（NG検知・通報）。**コンシェルジュ(公式)** スレッド。
            まとめてギフト導線。
[呼ぶ]★    ホーム。**今すぐ呼ぶ（パトコール）** / 選んで呼ぶ（指名, 現在のキャスト数）
            └ 呼び出し作成: エリア/合流予定時刻/設定時間/クラス×人数/詳細条件
              → 確認(CastRequestConfirm) → 初回注意喚起(FirstCallWarning) → 成立待ち
            注文履歴の確認、**今日会えるキャスト**（在席で抽出・横スクロール）
[つぶやき]   キャストの短文投稿（コパト導線、次フェーズ）
[マイページ] プロフィール、ポイント購入/残高、支払い方法、領収書、バッジ/グレード、
            利用準備チェックリスト、お知らせ、友達招待コード、ランキング
```

- 各画面は Blade + Vite。動的部分（待機キャスト数、成立待ち、チャット）は最小 API + JS で更新。
- 認証前でもキャスト閲覧可（`no-login-search`）。呼び出し・メッセージは要ログイン＋本人確認。
- 呼び出しフローは **確認→初回注意喚起→与信ホールド→公開→成立** を Service で一貫制御。
